...
func (mwt *nftMetadataWorkerThread) update(tok *types.Token) {
	...
	// get the metadata
	log.Infof("loading metadata for %s/%s", tok.Contract.String(), tok.TokenId.String())
	...
	if md.Image != nil && *md.Image != tok.ImageURI {
		...
		if err != nil || img == nil {
			if err != nil {
				...
				log.Warningf("NFT image [%s] not found on %s/%s", *md.Image, tok.Contract.String(), tok.TokenId.String())
			}
			mwt.tryLegacyUpdate(tok)
		...
		log.Infof("NFT image [%s] has type %d", *md.Image, img.Type)
